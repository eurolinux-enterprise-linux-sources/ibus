...
static void application_config_name_owner_changed_cb (Application* self, GDBusConnection* connection, const gchar* sender_name, const gchar* object_path, const gchar* interface_name, const gchar* signal_name, GVariant* parameters);
static void _application_config_name_owner_changed_cb_gd_bus_signal_callback (GDBusConnection* connection, const gchar* sender_name, const gchar* object_path, const gchar* interface_name, const gchar* signal_name, GVariant* parameters, gpointer self);
...
static void _application_config_name_owner_changed_cb_gd_bus_signal_callback (GDBusConnection* connection, const gchar* sender_name, const gchar* object_path, const gchar* interface_name, const gchar* signal_name, GVariant* parameters, gpointer self) {
	application_config_name_owner_changed_cb (self, connection, sender_name, object_path, interface_name, signal_name, parameters);
}
...
static void application_init (Application* self) {
	...
	self->priv->m_config = _tmp6_;
	g_dbus_connection_signal_subscribe (connection, "org.freedesktop.DBus", "org.freedesktop.DBus", "NameOwnerChanged", "/org/freedesktop/DBus", IBUS_SERVICE_CONFIG, G_DBUS_SIGNAL_FLAGS_NONE, _application_config_name_owner_changed_cb_gd_bus_signal_callback, application_ref (self), application_unref);
...
static void application_bus_name_acquired_cb (Application* self, GDBusConnection* connection, const gchar* sender_name, const gchar* object_path, const gchar* interface_name, const gchar* signal_name, GVariant* parameters) {
	...
	_tmp0_ = signal_name;
	g_debug ("application.vala:87: signal_name = %s", _tmp0_);
	_tmp1_ = self->priv->m_bus;
	...
	if (_tmp3_ != NULL) {
		Panel* _tmp4_;
		IBusConfig* _tmp5_;
		_tmp4_ = self->priv->m_panel;
		_tmp5_ = self->priv->m_config;
		panel_set_config (_tmp4_, _tmp5_);
	}
}
...
static void application_bus_name_lost_cb (Application* self, GDBusConnection* connection, const gchar* sender_name, const gchar* object_path, const gchar* interface_name, const gchar* signal_name, GVariant* parameters) {
	...
	g_return_if_fail (parameters != NULL);
	_tmp0_ = signal_name;
	g_debug ("application.vala:101: signal_name = %s", _tmp0_);
	_g_object_unref0 (self->priv->m_panel);
...
static void application_config_name_owner_changed_cb (Application* self, GDBusConnection* connection, const gchar* sender_name, const gchar* object_path, const gchar* interface_name, const gchar* signal_name, GVariant* parameters) {
	...
	g_return_if_fail (parameters != NULL);
	_tmp0_ = signal_name;
	g_debug ("application.vala:111: signal_name = %s", _tmp0_);
	_tmp1_ = parameters;
	g_variant_get (_tmp1_, "(sss)", &name, &old_owner, &new_owner, NULL);
	_tmp2_ = new_owner;
	if (g_strcmp0 (_tmp2_, "") == 0) {
		...
		IBusBus* _tmp3_;
		IBusConfig* _tmp4_ = NULL;
		IBusConfig* _tmp5_;
		_tmp3_ = self->priv->m_bus;
		_tmp4_ = ibus_bus_get_config (_tmp3_);
		_tmp5_ = _g_object_ref0 (_tmp4_);
		_g_object_unref0 (self->priv->m_config);
		self->priv->m_config = _tmp5_;
	}
	_tmp6_ = self->priv->m_panel;
	if (_tmp6_ != NULL) {
		Panel* _tmp7_;
		IBusConfig* _tmp8_;
		_tmp7_ = self->priv->m_panel;
		_tmp8_ = self->priv->m_config;
		panel_set_config (_tmp7_, _tmp8_);
	}
	_g_free0 (old_owner);
	_g_free0 (new_owner);
	_g_free0 (name);
...
static void application_bus_disconnected (Application* self, IBusBus* bus) {
	...
	g_debug ("application.vala:125: connection is lost.");
	gtk_main_quit ();
}
