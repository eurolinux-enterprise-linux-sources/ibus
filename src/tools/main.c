...
struct _CommandEntry {
	...
	GDestroyNotify entry_target_destroy_notify;
};
...
void command_entry_copy (const CommandEntry* self, CommandEntry* dest);
void command_entry_destroy (CommandEntry* self);
...
const CommandEntry commands[9] = {{"engine", _get_set_engine_entry_func}, {"exit", _exit_daemon_entry_func}, {"list-engine", _list_engine_entry_func}, {"watch", _message_watch_entry_func}, {"restart", _restart_daemon_entry_func}, {"version", _print_version_entry_func}, {"read-cache", _read_cache_entry_func}, {"write-cache", _write_cache_entry_func}, {"help", _print_help_entry_func}};
...
void command_entry_copy (const CommandEntry* self, CommandEntry* dest) {
	...
	(*dest).name = _tmp1_;
	_tmp2_ = (*self).entry;
	_tmp2__target = (*self).entry_target;
	((*dest).entry_target_destroy_notify == NULL) ? NULL : ((*dest).entry_target_destroy_notify ((*dest).entry_target), NULL);
	(*dest).entry = NULL;
	(*dest).entry_target = NULL;
	(*dest).entry_target_destroy_notify = NULL;
	(*dest).entry = _tmp2_;
	(*dest).entry_target = _tmp2__target;
	(*dest).entry_target_destroy_notify = NULL;
}
...
void command_entry_destroy (CommandEntry* self) {
	_g_free0 ((*self).name);
	((*self).entry_target_destroy_notify == NULL) ? NULL : ((*self).entry_target_destroy_notify ((*self).entry_target), NULL);
	(*self).entry = NULL;
	(*self).entry_target = NULL;
	(*self).entry_target_destroy_notify = NULL;
}
...
CommandEntry* command_entry_dup (const CommandEntry* self) {
	...
	command_entry_copy (self, dup);
	return dup;
}


void command_entry_free (CommandEntry* self) {
	command_entry_destroy (self);
	g_free (self);
}
